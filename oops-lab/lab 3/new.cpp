...
string checkRowWords(char **arr,  int numberOfRows, int numberOfColumns , int ); 
...
int main(){
    ...
    //the possible combinations are numberOfColumns!(factorial)
    int numberOfLettersTocheck=1;
    for(int i = 0 ; i < numberOfRows ; i++){
        for(int j = 0 ; j < numberOfColumns ; j++){
            for(int k =1;k<=numberOfLettersTocheck;k++){
            checkRowWords(arr,i,j,k);
            //findWord(combination);
            }
            numberOfLettersTocheck++;
        }
    }
...
string checkRowWords(char **arr,  int rowNumber, int startingIndex, int numberOfLettersTocheck ){
    string combinationSentence="";
    for(int i=startingIndex ; i<numberOfLettersTocheck ;i++){
        combinationSentence+=arr[rowNumber][i];
    }
    cout<<combinationSentence<<endl;
    return combinationSentence;
}
void findWord(string combinationSentence){
    for (int i=0 ; i<110;i++)
